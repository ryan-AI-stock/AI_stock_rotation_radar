...
TASK = "TASK-RADAR-DATA-VNEXT-P1-P2-OLD-AI7-VS-00631L-WEEKLY-MEDIAN-RS60-TWO-SLEEVE-RECHAIN-EXACT-CLOSE-FILL-002"
...
OUTPUT = REPO / "outputs/radar_vnext_p1_p2_old_ai7_median_rs60_two_sleeve_rechain_close_fill_20260719"
TARGETS = {"2019-04-02", "2023-05-26"}
REUSE = {
    "2019-04": REPO / "outputs/radar_vnext_p1_p2_strict_bear_cash_final_union_close_fill_20260718/checkpoints/00631L/2019-04.json.gz",
...
def run(args: argparse.Namespace) -> dict:
    ...
    prefix = "old_ai7_median_rs60_two_sleeve_rechain_exact_close"
    ...
    atomic_json(output / "readiness_for_core_old_ai7_median_rs60_two_sleeve_rechain_close_absorption.json", readiness)
    atomic_json(output / "progress.json", {**readiness, "updated_at": now()})
    atomic_text(output / "current_step.txt", "status=complete\nresume_step=none\nnext_owner=Core_Data_two_sleeve_rechain\n")
    atomic_text(output / "final_summary_zh.md", f"# old AI7 versus 00631L weekly median RS60 two-sleeve rechain exact close fill\n\n- authority=2；official raw close={len(patch)}；official no-trade={len(no_trade_frame)}；blocked=0。\n- local accepted official checkpoint reuse only；未啟動新網路。\n")
