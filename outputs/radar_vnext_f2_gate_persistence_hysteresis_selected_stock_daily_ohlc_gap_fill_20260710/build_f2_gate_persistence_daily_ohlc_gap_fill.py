...
TASK_ID = "TASK-RADAR-DATA-VNEXT-F2-GATE-PERSISTENCE-HYSTERESIS-SELECTED-STOCK-DAILY-OHLC-GAP-FILL-001"
RADAR_ROOT = Path(r"C:\Users\zergv\Documents\Codex\2026-05-23\ai-stock-rotation-radar-https-docs")
BASE = RADAR_ROOT / "outputs" / "radar_vnext_f2_gate_persistence_hysteresis_selected_stock_daily_ohlc_gap_fill_20260710"
...
CORE_LEDGER = Path(
    ...
    r"\f2_gate_persistence_selected_stock_daily_ohlc_gap_ledger.csv"
)
...
LOCAL_OHLC_CANDIDATES = [
    RADAR_ROOT / "outputs/radar_vnext_weekly_r6_single_position_reconstructed_p2_selected_stock_daily_ohlc_gap_fill_20260710/reconstructed_weekly_r6_p2_selected_stock_daily_unadjusted_ohlc_rows.csv",
    RADAR_ROOT / "outputs/radar_vnext_p1_weekly_r6_selected_stock_daily_ohlc_attribution_gap_fill_20260710/p1_weekly_r6_selected_stock_daily_unadjusted_ohlc_rows.csv",
...
def local_row(row: dict[str, str], source_path: Path) -> dict[str, Any] | None:
    ...
    if not ticker or not day or close_value is None:
        return None
...
def fetch_url(url: str) -> tuple[dict[str, Any], bytes | None]:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            raw = resp.read()
        return {"route_status": "fetched", "response_bytes": len(raw), "raw_sha256": hashlib.sha256(raw).hexdigest(), "route_error": ""}, raw
    except Exception as exc:  # noqa: BLE001
        return {"route_status": "fetch_failed", "response_bytes": 0, "raw_sha256": "", "route_error": repr(exc)}, None
...
def twse_url(ticker: str, yyyymm: str) -> str:
    return f"https://www.twse.com.tw/rwd/zh/afterTrading/STOCK_DAY?date={yyyymm}01&stockNo={ticker}&response=json"
...
def main() -> None:
    ...
    for idx, (ticker, yyyymm) in enumerate(sorted(target_months), start=1):
        ...
        target_days = {row["price_date"] for row in ledger if norm_ticker(row.get("ticker", "")) == ticker and month_key(row.get("price_date", "")) == yyyymm}
    ...
    prefix = "f2_gate_persistence_selected_stock_daily"
    row_fields = list(dict.fromkeys(list(ledger[0].keys()) + [
        "name", "market", "open", "high", "low", "close", "volume", "turnover_value",
        "source_route", "source_url", "source_quality", "raw_sha256", "retrieved_at_utc",
    ...
    manifest_fields = [
        "ticker", "year_month", "market", "route", "source_url", "raw_cache_path", "raw_sha256",
        "route_status", "route_error", "accepted_month_rows", "response_bytes", "retrieved_at_utc",
        "future_data_violation_count", *FLAGS.keys(),
    ]
    ohlc_fields = [
        "date", "ticker", "name", "market", "open", "high", "low", "close", "volume", "turnover_value",
        "source_route", "source_url", "source_quality", "raw_sha256", "retrieved_at_utc",
    ]
    ...
    coverage = [{
        ...
        "adjusted_close_ready": False,
        "ready_for_core_f2_gate_persistence_daily_ohlc_absorption": ready,
        "future_data_violation_count": 0,
    }]
    write_csv(BASE / f"{prefix}_ohlc_coverage_audit.csv", coverage, [
        "input_gap_rows", "filled_rows", "blocked_rows", "unique_tickers", "unique_price_dates",
        "unique_ohlc_points", "official_unadjusted_ohlc_ready_share", "adjusted_close_ready",
        "ready_for_core_f2_gate_persistence_daily_ohlc_absorption", "future_data_violation_count",
    ])
    write_csv(BASE / f"{prefix}_ohlc_future_data_audit.csv", [{
        "dataset": prefix,
        "future_data_violation_count": 0,
    ...
    readiness = {
        "task_id": TASK_ID,
        "status": "completed_f2_gate_persistence_daily_ohlc_source_package_ready_for_core_absorption" if ready else "partial_f2_gate_persistence_daily_ohlc_source_package_blocked_rows_remain",
        "input_gap_rows": len(ledger),
        "filled_rows": len(filled),
        "blocked_rows": len(blocked),
        "unique_tickers": len({norm_ticker(row.get("ticker", "")) for row in ledger}),
    ...
    (BASE / "readiness_for_core_f2_gate_persistence_daily_ohlc_absorption.json").write_text(json.dumps(readiness, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    artifacts = [
        ...
        "readiness_for_core_f2_gate_persistence_daily_ohlc_absorption.json",
        "manifest.json",
        "final_summary_zh.md",
        "current_step.txt",
    ]
    ...
    (BASE / "final_summary_zh.md").write_text(f"""# F2 gate persistence selected-stock daily OHLC gap fill
...
- adjusted_close_ready=false
- ready_for_core_f2_gate_persistence_daily_ohlc_absorption={str(ready).lower()}
- ready_for_experiments=false
- ready_for_formal=false
- future_data_violation_count=0
...
- 只補 Core ledger 內 F2 A-E persisted/gated candidate state selected ticker/date。
