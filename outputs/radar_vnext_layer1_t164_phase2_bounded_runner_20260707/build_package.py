...
CORE_CONTRACT = Path(r"C:\Users\zergv\Documents\Codex\2026-05-30\ep05-chat-ai-stock-backtest-lab\outputs\vnext_layer1_t164_phase2_bounded_expansion_contract_20260707")

TASK_ID = "TASK-RADAR-DATA-VNEXT-LAYER1-T164-PHASE2-BOUNDED-RUNNER-001"
...
PERIODS = [
    ...
    {"report_period": "114Q4", "roc_year": "114", "season": "4", "expected_query_months": [("115", "02"), ("115", "03"), ("115", "04")]},
    {"report_period": "114Q3", "roc_year": "114", "season": "3", "expected_query_months": [("114", "10"), ("114", "11")]},
    {"report_period": "114Q2", "roc_year": "114", "season": "2", "expected_query_months": [("114", "07"), ("114", "08")]},
]
...
def period_tokens(period):
    if period == "115Q1":
        return ["115年第1季", "115年第一季", "115年度第1季", "115年度第一季", "一一五年第一季", "115/01/01~115/03/31", "第1季財務報表", "第一季財務報表"]
    if period == "114Q4":
        return ["114年第4季", "114年第四季", "一一四年第四季", "114年度", "一一四年度", "114/01/01~114/12/31", "114年度財務報表", "年度財務報表"]
    if period == "114Q3":
        return ["114年第3季", "114年第三季", "114年度第3季", "114年度第三季", "一一四年第三季", "114/01/01~114/09/30", "第3季財務報表", "第三季財務報表"]
    return ["114年第2季", "114年第二季", "114年度第2季", "114年度第二季", "一一四年第二季", "114/01/01~114/06/30", "第2季財務報表", "第二季財務報表"]
...
def wrong_period_tokens(period):
    ...
    if period == "114Q3":
        return ["115年第1季", "115年第一季", "114年度", "114年第一季", "114年第二季", "114年第四季", "113年"]
    return ["115年第1季", "115年第一季", "114年度", "114年第一季", "114年第三季", "114年第四季", "113年"]


def subject_prefilter(subject, period):
    text = clean(subject)
    if any(token in text for token in ["預計召開日期", "召開日期", "預計提報", "提報董事會日期", "提請董事會決議日期"]):
        return "excluded_premeeting_notice", False
    if not any(token in text for token in ["財務報告", "合併財報", "合併財務報告", "財務報表", "財報", "董事會重要決議", "董事會決議"]):
...
def classify_candidate(subject, detail_text, period):
    ...
    financial_subject = any(token in text for token in ["財務報告", "合併財報", "合併財務報告", "財務報表", "財報"])
...
CONTRACT_ROWS = read_csv(CORE_CONTRACT / "layer1_t164_phase2_bounded_runner_contract.csv")
TICKERS = sorted(
    {
...
projected_rows = [{
    "scope": "phase2_tpex_100x4_bounded_runner",
...
if projected_rows[0]["budget_status"] != "pass":
    ...
    write_json("readiness_for_core_t164_phase2_bounded_runner.json", {
        "task_id": TASK_ID,
        "status": "blocked_by_projected_route_budget",
        "ready_for_core_t164_phase2_materialization_review": False,
...
for ticker_info in TICKER_ROWS:
    for period in PERIODS:
        ...
        contract_audit_rows.append({
            ...
            "runner_scope": "phase2_tpex_100x4_bounded",
            **FLAGS,
        })
...
tpex_readiness = [{
    ...
    "reason": "TPEx phase2 100x4 bounded runner executed; current-or-carried sampling universe is not historical PIT all-stock universe and full-period route remains incomplete",
    **FLAGS,
}]
...
readiness = {
    ...
    "coverage": "100 TPEx tickers x 4 periods phase2 bounded runner; current-or-carried sampling universe only; not historical PIT all-stock universe",
    ...
    "budget_routes_per_row": MAX_PROJECTED_ROUTES_PER_ROW,
    "ready_for_core_t164_phase2_materialization_review": statement_success_rows == total_rows and route_error_count == 0,
    ...
    "future_data_violation_count": 0,
    "not_live_rule": True,
    "blocked_reason": f"phase2 bounded runner has {total_rows - asof_matched_rows} official-asof blocked rows; historical PIT all-stock universe, full 46-quarter range, full universe materialization, capex formal label, and receivables formal label remain incomplete",
    **FLAGS,
}
...
manifest = {
    ...
    "ready_for_strategy_replay": False,
    "ready_for_core_t164_phase2_materialization_review": readiness["ready_for_core_t164_phase2_materialization_review"],
    "ready_for_experiments": False,
    "ready_for_formal": False,
    **FLAGS,
    "files": [
        ...
        "phase2_t164_tpex_materialized_field_matrix.csv",
        "phase2_official_asof_candidate_ledger.csv",
        "phase2_official_asof_candidate_evidence.csv",
        ...
        "phase2_contract_execution_audit.csv",
        "future_data_governance_audit.csv",
        "blocked_or_ambiguous_rows.csv",
        "readiness_for_core_t164_phase2_bounded_runner.json",
        "manifest.json",
        "final_summary_zh.md",
    ],
}
...
write_csv("phase2_t164_tpex_materialized_field_matrix.csv", materialized_rows)
write_csv("phase2_official_asof_candidate_ledger.csv", asof_rows)
write_csv("phase2_official_asof_candidate_evidence.csv", candidate_rows)
...
write_csv("phase2_contract_execution_audit.csv", contract_audit_rows)
write_csv("future_data_governance_audit.csv", future_rows)
write_csv("blocked_or_ambiguous_rows.csv", blocked_rows)
write_json("readiness_for_core_t164_phase2_bounded_runner.json", readiness)
write_json("manifest.json", manifest)

summary = f"""# Layer1 t164 TPEx phase_2 100x4 bounded runner
...
Readiness:
- ready_for_core_t164_phase2_materialization_review={str(readiness['ready_for_core_t164_phase2_materialization_review']).lower()}
- ready_for_core_t164_broader_ingest_contract=false
- ready_for_core_t164_broader_materialization=false
- ready_for_experiments=false
- ready_for_formal=false
